...
uint Directory::moveFile(uint pn, byte *srcleaf, byte *dstleaf)
{
  if (this->iNumberOf(dstleaf) != 0)
  {
    return 0; // destination name can't already exist in this dir
  }

  Directory *d = new Directory(fv, pn, 0);
  uint in = d->iNumberOf(srcleaf);
  if (in > 0)
  {
    addLeafName(dstleaf, in);
    d->deleteFile(srcleaf, 0);
  }
